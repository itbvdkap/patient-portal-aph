...
const schema = z.object({
  ...
  branchCode: z.enum(["CN1", "CN3"]).optional(), branch_code: z.enum(["CN1", "CN3"]).optional(), branchId: z.union([z.number(), z.string().trim()]).optional(), branch: z.string().trim().optional().or(z.literal("")), soCCCD: z.string().trim().optional().or(z.literal("")), ngayCap: z.string().trim().optional().or(z.literal("")),
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!branchCode) return NextResponse.json({ message: "Vui lòng chọn chi nhánh." }, { status: 400 });
    const result = await createBookingAppointment({ ...parsed.data, oldPatientCode: parsed.data.oldPatientCode || parsed.data.patientCode, branchCode, accountKey: session.accountId ?? session.accountKey });
