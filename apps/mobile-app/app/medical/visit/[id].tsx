...
export default function VisitDetailScreen() {
  ...
  return <Screen nav><ScrollView contentContainerStyle={styles.container}>
    ...
    <View style={styles.heading}><H1>Chi tiết lần khám</H1><Body>{formatDateTime(visit.visitDate)} · {normalizeDisplayText(visit.departmentName) || "Tiếp đón/KCB"}</Body></View>
    <Card><H2>Tổng quan</H2><Info label="Phòng" value={normalizeDisplayText(visit.departmentName)} /><Info label="Bác sĩ" value={normalizeDisplayText(visit.doctorName)} /><Info label="Mã lượt" value={visit.hisVisitId} /><Info label="Hẹn tái khám" value={visit.followUpDate ? formatDate(visit.followUpDate) : "Chưa ghi nhận"} /></Card>
    <Card><H2>Chẩn đoán</H2><Info label="CD ra viện" value={normalizeDisplayText(visit.primaryDiagnosis)} />{visit.secondaryDiagnosis ? <Info label="Chẩn đoán kèm theo" value={normalizeDisplayText(visit.secondaryDiagnosis)} /> : null}<Body>{normalizeDisplayText(visit.notes) || "Chưa có ghi chú."}</Body></Card>
    ...
    <Card><H2>Chỉ định</H2>{visit.services.length ? visit.services.map((service) => <View key={service.id} style={styles.item}><Text style={styles.itemTitle}>{normalizeDisplayText(service.serviceName)}</Text><Mono>{formatDateTime(service.performedAt)} · {normalizeDisplayText(service.status)}</Mono></View>) : <EmptyState text="Chưa có chỉ định dịch vụ." />}</Card>
    <Card><H2>Kết quả xét nghiệm</H2>{visit.labResults.length ? visit.labResults.map((lab) => <View key={lab.id} style={styles.lab}><Text style={styles.labName}>{normalizeDisplayText(lab.testName)}</Text><Text style={styles.labValue}>{String(lab.result)} {normalizeDisplayText(lab.unit)}</Text><Text style={styles.labRef}>{normalizeDisplayText(lab.referenceRange) || "-"}</Text><Text style={lab.flag === "Bình thường" ? styles.normal : styles.high}>{normalizeDisplayText(lab.flag)}</Text></View>) : <EmptyState text="Chưa có kết quả xét nghiệm." />}</Card>
    <Card><H2>Chẩn đoán hình ảnh</H2>{visit.imagingResults.length ? visit.imagingResults.map((item) => <View key={item.id} style={styles.item}><Text style={styles.itemTitle}>{normalizeDisplayText(item.techniqueName)}</Text><Mono>{formatDate(item.date)}</Mono><Body>{normalizeDisplayText(item.conclusion) || "Chưa có kết luận."}</Body></View>) : <EmptyState text="Chưa có kết quả CĐHA." />}</Card>
    <Card><H2>Đơn thuốc</H2>{visit.prescription?.items?.length ? visit.prescription.items.map((item) => <View key={item.id} style={styles.item}><Text style={styles.itemTitle}>{normalizeDisplayText(item.medicineName)}</Text><Body>{item.quantity} · {normalizeDisplayText(item.dosage)}</Body></View>) : <EmptyState text="Không có đơn thuốc trong lần khám này." />}</Card>
    <Card><H2>Lời dặn bác sĩ</H2><Body>{normalizeDisplayText(visit.doctorAdvice) || "Chưa có lời dặn."}</Body></Card>
